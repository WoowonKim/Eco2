...
const CateOneList = ({
  ...
  const result = favoriteArr.filter(it => it.category === 1);
  console.log("result ===> ", result);
  const [starTrue, setStartTrue] = useState(false);
  const [star, setStar] = useState(0);
  const resultStar = result.filter(it => it.id === star);
  console.log("TEST--->", resultStar);
  return (
    <div>
      {/* <p
